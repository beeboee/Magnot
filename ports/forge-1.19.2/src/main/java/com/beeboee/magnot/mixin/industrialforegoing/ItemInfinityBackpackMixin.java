...
public abstract class ItemInfinityBackpackMixin {
    ...
    private <T extends Entity> List<T> magnot$filterBackpackItems(
        ...
        if (!(level instanceof ServerLevel) || !ItemEntity.class.isAssignableFrom(entityClass)) {
            return candidates;
        }
        ServerLevel serverLevel = (ServerLevel) level;
        return candidates.stream()
                .filter(candidate -> !(candidate instanceof ItemEntity)
                        || !magnot$isBlocked(serverLevel, source, (ItemEntity) candidate))
                .collect(Collectors.toList());
    }

    private static boolean magnot$isBlocked(ServerLevel level, Entity source, ItemEntity item) {
