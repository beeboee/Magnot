...
public abstract class ItemInfinityBackpackMixin {
    @Redirect(method = "inventoryTick", at = @At(value = "INVOKE", target = "Lnet/minecraft/world/level/Level;getEntitiesOfClass(Ljava/lang/Class;Lnet/minecraft/world/phys/AABB;)Ljava/util/List;"), require = 0)
    private <T extends Entity> List<T> magnot$filterBackpackItems(Level level, Class<T> type, AABB box, ItemStack stack, Level originalLevel, Entity source, int slot, boolean selected) {
        ...
        return candidates.stream().filter(candidate -> !(candidate instanceof ItemEntity)
                || !magnot$isProtected(serverLevel, source, (ItemEntity) candidate)).collect(Collectors.toList());
    }

    private static boolean magnot$isProtected(ServerLevel level, Entity source, ItemEntity item) {
        if (source instanceof Player) return FerrousMagnetRules.blocksPlayerItemPull(level, (Player) source, item);
