...
public abstract class FortuneCoinItemMixin {
    ...
    private <T extends Entity> List<T> magnot$filterPlayerItems(Level level, Class<T> type, AABB box,
                                                                Level originalLevel, Player player,
                                                                double distance) {
    ...
    private <T extends Entity> List<T> magnot$filterPedestalItems(Level level, Class<T> type, AABB box,
                                                                  @Coerce Object pedestal,
                                                                  Level originalLevel, BlockPos pos) {
        List<T> candidates = level.getEntitiesOfClass(type, box);
        if (!(level instanceof ServerLevel) || !ItemEntity.class.isAssignableFrom(type)) {
            return candidates;
        }
