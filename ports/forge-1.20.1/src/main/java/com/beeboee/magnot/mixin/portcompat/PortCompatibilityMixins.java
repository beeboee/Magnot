...
abstract class DraconicEvolutionMagnetMixin {
    ...
    private <T extends Entity> List<T> magnot$filterDraconicItems(Level level, Class<T> type, AABB box, ItemStack stack, Entity source) {
        ...
        return candidates.stream().filter(candidate -> !(candidate instanceof ItemEntity item)
                || !FerrousMagnetRules.blocksPlayerItemPull(serverLevel, player, item)).toList();
    }
}
...
abstract class ReliquaryFortuneCoinMixin {
    ...
    private <T extends Entity> List<T> magnot$filterReliquaryPedestalItems(Level level, Class<T> type, AABB box, @Coerce Object pedestal, Level originalLevel, BlockPos pos) {
        ...
        return candidates.stream().filter(candidate -> !(candidate instanceof ItemEntity item)
                || !FerrousMagnetRules.blocksItemPull(serverLevel, source, item)).toList();
    }
}
...
abstract class EnderIoElectromagnetMixin {
    @Redirect(method = "onTickWhenActive", at = @At(value = "INVOKE", target = "Lnet/minecraft/world/level/Level;getEntities(Lnet/minecraft/world/entity/Entity;Lnet/minecraft/world/phys/AABB;Ljava/util/function/Predicate;)Ljava/util/List;", remap = true), require = 0)
...
abstract class EnderIoVacuumMachineMixin {
    @Redirect(method = "getEntities", at = @At(value = "INVOKE", target = "Lnet/minecraft/world/level/Level;getEntitiesOfClass(Ljava/lang/Class;Lnet/minecraft/world/phys/AABB;Ljava/util/function/Predicate;)Ljava/util/List;", remap = true), require = 0)
